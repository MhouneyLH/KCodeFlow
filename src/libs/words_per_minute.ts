...
export function getAverageWordsPerMinute(repository: KeystrokeRepository): number {
  const keystrokesPerSecond: number = repository.second.count;
  ...
  const averageWordsPerMinute =
    wordsPerMinuteEstimations.reduce((prev, curr) => prev + curr) /
    wordsPerMinuteEstimations.length;
